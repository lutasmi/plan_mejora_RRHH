...
export default function Card({ card, owner, tags, isSel, isHl, isDim, isFiltered, canEdit, onSelect, onHlCard, onDragStart }) {
  // ── DIAGNÓSTICO 5: ¿Card component se monta? ──────────────────────────
  console.log('[DIAG-5] Card mounted:', card.id, card.name, { isSel, isHl, isDim, isFiltered, opacity: isFiltered ? 0.06 : isDim ? 0.15 : 1 })
      ...
      <div style={{ display:'flex', gap:5, alignItems:'flex-start', marginBottom:5 }}>
        ...
        <button
          ...
          style={{ background:'none', border:`1px solid ${isHl?'#f0883e':'#21262d'}`,
            color:isHl?'#f0883e':'#30363d', cursor:'pointer',
            width:18, height:18, borderRadius:3, fontSize:9, padding:0, flexShrink:0,
