...
import {
  getPillarX, getRowY,
  totalCanvasW, totalCanvasH, computedRows,
...
export default function Canvas({
  ...
  const onBgDbl = e => {
    ...
    const ci = columns.findIndex((_, i) => { const px = getPillarX(columns, i); return cx >= px && cx < px + columns[i].w })
  ...
  // Card drag — starts on card div, captured by container
  ...
  return (
    ...
    >
      ...
      <svg
        ...
        onClick={e => {
          const t = e.target
          if (['svg','rect','line','text'].includes(t.tagName)) onSelectCard?.(null)
        }}
      >
        <defs>
        ...
        <g transform={`translate(${vp.x},${vp.y}) scale(${vp.scale})`}>
          ...
          {columns.map((col, ci) => {
            ...
            return (
              <g key={col.id}>
                ...
                <text x={cx + col.w / 2} y={HDR_H / 2} textAnchor="middle" dominantBaseline="middle"
                ...
                {canEdit && (
                  <rect x={cx + col.w - 5} y={0} width={10} height={TH}
          ...
          {/* Flechas de dependencia — sin bridges, sin ⊕ */}
          {arrows.map(a => {
      ...
      {canEdit && (
        <div style={{ position: 'absolute', bottom: 12, left: '50%', transform: 'translateX(-50%)', fontSize: 9, color: '#21262d', letterSpacing: '1px', pointerEvents: 'none', whiteSpace: 'nowrap' }}>
